...
export default function AddIncome({ addIncome }) {
  ...
  const handleSubmit = async (data, file) => {
    setErrorMsg(null)
    const { error } = await addIncome(data, file)
    if (error) {
      setErrorMsg(error)
    } else {
      setSuccess(true)
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">Pemasukan berhasil dicatat</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="pt-2">
        ...
        <p className="text-sm text-gray-500 mt-1">Tambahkan sumber pemasukan kamu</p>
      </div>
      ...
      <div className="card">
        <IncomeForm key={new Date().toISOString().split('T')[0]} onSubmit={handleSubmit} />
      </div>
    </div>
  )
}
