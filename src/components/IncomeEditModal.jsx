...
export default function IncomeEditModal({ income, onClose, onSave }) {
  const handleSubmit = async (data, file) => {
    const { error } = await onSave(income.id, data, file)
    if (!error) onClose()
  }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pb-3 sticky top-6 bg-[#1a1a24] z-10">
          <h2 className="text-base font-bold text-white">Edit Pemasukan</h2>
          <button
            onClick={onClose}
        ...
        <div className="px-5 pb-6">
          <IncomeForm initialData={income} onSubmit={handleSubmit} onCancel={onClose} />
        </div>
      </div>
    </div>
  )
